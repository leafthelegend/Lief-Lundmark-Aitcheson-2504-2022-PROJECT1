...
"""
function factor(f::Polynomial, prime::Integer)::Vector{Tuple{Polynomial,Integer}}
    ...
    ret_val = Tuple{Polynomial,Integer}[]
...
"""
function expand_factorization(factorization::Vector{<:Tuple{Polynomial,Integer}})::Polynomial 
...
"""
function multiplicity(f::Polynomial, g::Polynomial, prime::Integer)::Integer
...
"""
function dd_factor(f::Polynomial, prime::Integer)::Array{Polynomial}
    x = x_poly(typeof(f))
    w = copyTerms(x)
...
"""
function dd_split(f::Polynomial, d::Integer, prime::Integer)::Vector{Polynomial}
    f = mod(f,prime)
    degree(f) == d && return [f]
    degree(f) == 0 && return []
    w = rand(typeof(f), degree = d, monic = true)
    w = mod(w,prime)
    n_power = (prime^d-1) ÷ 2
